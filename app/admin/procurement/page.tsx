    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-3">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          ...
          <p className="text-3xl font-heading font-black text-[#0d3a24] mt-2">EGP {(stats.budget_used / 1000).toFixed(1)}k</p>
          <div className="mt-3 h-2 bg-[#e8f3ec] rounded-full">
            <div className={`h-full rounded-full ${budgetPct > 85 ? "bg-amber-500" : "bg-[#17583a]"}`} style={{ width: `${budgetPct}%` }} />
          </div>
          <p className="text-xs text-[#8aab99] mt-2">of EGP {(stats.budget_allocated / 1000).toFixed(0)}k allocated ({budgetPct}%)</p>
        </div>
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          <p className="text-xs font-semibold tracking-[0.1em] uppercase text-[#8aab99]">Remaining Budget</p>
          <p className="text-3xl font-heading font-black text-[#0d3a24] mt-2">EGP {((stats.budget_allocated - stats.budget_used) / 1000).toFixed(1)}k</p>
